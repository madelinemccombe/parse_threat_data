...
# output csv filename is query_type specific + scope and verdict appends
    if query_type == 'group':
        ...
        filename = 'tag_group_data_weekly_{0}_{1}.csv'.format(verdict,scope)

    if query_type == 'upload_source':
        query_field = 'session.upload_src'
        query_list = upload_source
        filename =  'upload_source_data_weekly_{0}_{1}.csv'.format(verdict,scope)

    if query_type == 'region':
        query_field = 'session.region'
        query_list = region
        filename = 'region_data_weekly_{0}_{1}.csv'.format(verdict,scope)
