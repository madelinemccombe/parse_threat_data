...
def cli(filename):
    ...
    output_filename = 'metrics_threat_output/metrics_threat_samples'
    skip_fields = ['_id','_index','_score','_type', 'date']
    int_fields = [
        "metrics.threat.samples.verdict.all.all", "metrics.threat.samples.verdict.all.customer",
        "metrics.threat.samples.verdict.all.feed", "metrics.threat.samples.verdict.malware.feed",
        "metrics.threat.samples.verdict.malware.all", "metrics.threat.samples.verdict.malware.customer",
        ]
    float_fields = ["metrics.threat.samples.toxicity.all", "metrics.threat.samples.toxicity.all"]
    ...
    with open(filename) as f:
        ...
        for row in reader:
            ...
            index_tag_full = {}
            index_tag_inner = {}
            elk_index_name = f"metrics-threat-samples-{date_datetime.year}-{date_datetime.strftime('%m')}"
